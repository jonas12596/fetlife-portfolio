...
export default function Nav({ windowMin, handleWindowMin, handleStartBtn }: NavProps) {
  return (
    <>
      <nav className="fixed bottom-0 left-0 w-full h-8 bg-gradient-to-b from-[#418deb] to-[#245edc] flex items-center justify-between shadow-[0_-1px_3px_rgba(0,0,0,0.5)] border-t border-white/40 z-20">
        <div className="h-full flex items-center gap-3">
          <button onClick={handleStartBtn} className="w-[80px] flex items-center justify-center cursor-pointer gap-2 px-3 py-[2px] rounded-r h-8 font-bold text-white text-sm bg-gradient-to-b from-[#1c9a1f] to-[#2b8120] border border-[#126907] shadow-inner relative">
            <Image src={xpLogo} alt="About Windows" width={16} height={16} />
            <div className="absolute inset-0 rounded-r pointer-events-none">
              <div className="absolute top-0 left-0 w-full h-[50%] rounded-r bg-white/25 blur-[1px]"></div>
            </div>
            Start
          </button>
          ...
          <div
            ...
            <Image src={explorer} alt="About Windows" width={16} height={16} />
            My Computer
          </div>
        </div>
